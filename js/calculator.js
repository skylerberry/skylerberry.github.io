import {
  ...
  calculateRMultiple,
  calculateROI,
  parseDiscordAlert, // Added import for Discord alert parsing
  DEFAULTS,
  RISK_LEVELS,
} from "./utils.js"
...
export class Calculator {
  ...
  getElements() {
    return {
      inputs: {
        ...
        discordAlert: document.getElementById("discordAlert"), // Added Discord alert textarea
      },
      results: {
      ...
      errors: {
        ...
        parseAlert: document.getElementById("parseAlertError"), // Added parse alert error element
      },
      controls: {
  ...
  setupControlHandlers() {
    ...
    if (this.elements.controls.parseAlertButton) {
      this.elements.controls.parseAlertButton.addEventListener("click", () => {
        this.parseAndFillAlert()
      })
    }

    if (this.elements.inputs.discordAlert) {
      this.elements.inputs.discordAlert.addEventListener("keydown", (e) => {
  ...
  clearAll() {
    ...
    Object.entries(this.elements.inputs).forEach(([key, input]) => {
      ...
      } else if (key === "discordAlert") {
        // Added Discord alert clearing
        input.value = ""
      } else {
        input.value = ""
    ...
    this.resetResults()
    this.displayErrors([])
    this.displayParseError("") // Clear parse errors too
  ...
  parseAndFillAlert() {
    const alertText = this.elements.inputs.discordAlert.value.trim()

    // Clear previous errors
    this.displayParseError("")

    if (!alertText) {
      this.displayParseError("Please paste a Discord alert first")
      return
    }

    const parseResult = parseDiscordAlert(alertText)

    if (!parseResult.success) {
      this.displayParseError(parseResult.error)
      return
    }

    const { ticker, entryPrice, stopLossPrice, riskPercentage } = parseResult.data

    // Fill the calculator inputs
    this.elements.inputs.entryPrice.value = entryPrice.toString()
    this.elements.inputs.stopLossPrice.value = stopLossPrice.toString()
    this.elements.inputs.riskPercentage.value = riskPercentage.toString()

    // Update state
    this.updateStateFromInput("entryPrice", entryPrice)
    this.updateStateFromInput("stopLossPrice", stopLossPrice)
    this.updateStateFromInput("riskPercentage", riskPercentage)

    // Update risk button selection
    this.updateActiveRiskButton(riskPercentage)

    // Show success feedback
    this.showParseSuccess(ticker)

    // Calculate immediately
    this.calculate()

    console.log(
      `📊 Parsed Discord alert for ${ticker}: Entry $${entryPrice}, Stop $${stopLossPrice}, Risk ${riskPercentage}%`,
    )
  }

  displayParseError(message) {
    if (this.elements.errors.parseAlert) {
      updateElement(this.elements.errors.parseAlert, message)
      toggleClass(this.elements.errors.parseAlert, "hidden", !message)
    }
  }

  showParseSuccess(ticker) {
    // Clear any existing error
    this.displayParseError("")

    // Add success styling
    this.elements.inputs.discordAlert.classList.add("success")
    this.elements.controls.parseAlertButton.classList.add("success")

    // Clear the textarea after successful parse
    setTimeout(() => {
      this.elements.inputs.discordAlert.value = ""
      this.elements.inputs.discordAlert.classList.remove("success")
      this.elements.controls.parseAlertButton.classList.remove("success")
    }, 2000)

    // Show temporary success message
    this.displayParseError(`✅ Successfully parsed alert for $${ticker}`)
    setTimeout(() => {
      this.displayParseError("")
    }, 3000)
  }
