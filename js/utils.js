...
// ==========================================================================
// DISCORD ALERT PARSING UTILITIES
// ==========================================================================

export function parseDiscordAlert(alertText) {
  ...
  const patterns = [
    // Pattern 1: "Adding $TICKER shares @ $X.XX Stop loss @ $X.XX Risking X.XX%"
    /Adding\s+\$([A-Z]+)\s+shares\s+@\s+\$([0-9]+\.?[0-9]*)\s+Stop\s+loss\s+@\s+\$([0-9]+\.?[0-9]*)\s+Risking\s+([0-9]+\.?[0-9]*)%/i,

    // Pattern 2: Multi-line format
    /Adding\s+\$([A-Z]+)\s+shares\s+@\s+\$([0-9]+\.?[0-9]*)\s*[\r\n]+\s*Stop\s+loss\s+@\s+\$([0-9]+\.?[0-9]*)\s*[\r\n]+\s*Risking\s+([0-9]+\.?[0-9]*)%/i,

    // Pattern 3: More flexible spacing
    /Adding\s+\$([A-Z]+).*?@\s*\$([0-9]+\.?[0-9]*).*?Stop\s+loss.*?@\s*\$([0-9]+\.?[0-9]*).*?Risking\s+([0-9]+\.?[0-9]*)%/is,
  ]
  ...
  if (!match) {
    return {
      success: false,
      error: 'Could not parse alert. Expected format: "Adding $TICKER shares @ $X.XX Stop loss @ $X.XX Risking X.XX%"',
    }
  }

  const [, ticker, entryPrice, stopPrice, riskPercent] = match
  ...
  if (isNaN(entry) || isNaN(stop) || isNaN(risk)) {
    return {
      success: false,
      error: "Invalid numbers found in alert",
    }
  }
